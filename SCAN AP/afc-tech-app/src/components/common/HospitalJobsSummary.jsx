...
export default function HospitalJobsSummary({
  hospitalId,
  defaultStart,
  defaultEnd,
}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const ahuRes = await API.get("/ahus");
        const ahuJson = ahuRes.data || [];
        const ahuMap = new Map();
        for (const a of ahuJson) {
          ahuMap.set(String(a.id), {
        ...
        for (const j of filtered) {
          const ahuId = String(j.ahu_id);
          const ahuName =
            (ahuMap.get(ahuId) && ahuMap.get(ahuId).name) || j.ahu_name || ahuId;
  ...
  if (!hospitalId) {
    return (
      <div className="p-4 bg-base-100 border rounded">
        Select a hospital to view completed work.
      </div>
    );
  }
    ...
    <div className="p-4 bg-base-100 border rounded">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="ml-auto text-sm opacity-70">
          Showing {summary.reduce((acc, s) => acc + s.count, 0)} jobs across{" "}
          {summary.length} AHUs
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-sm opacity-70 py-4">
          No completed jobs for the selected range.
        </div>
      ) : (
                  ...
                  <div key={job.id} className="p-2 border rounded-sm bg-base-100">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-medium">
                        Job #{job.id} — {job.technician || "Unknown"}
                      </div>
                      <div className="text-xs opacity-60">
